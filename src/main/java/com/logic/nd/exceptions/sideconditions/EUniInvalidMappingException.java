...
public class EUniInvalidMappingException extends NDRuleException {
    ...
    public EUniInvalidMappingException(ASTEUni rule, ASTVariable variable, IASTExp from, IASTExp to, Set<IASTExp> outcomes) {
    ...
    protected String produceFeedback(FeedbackLevel level) {
        ...
        return switch (level) {
            ...
            case MEDIUM -> {
                rule.appendErrors(new NDTextException(EFeedbackPosition.CONCLUSION, "Invalid mapping!"));
                yield error;
            }
            case HIGH -> {
    ...
    public List<IASTND> getPreviews(FeedbackLevel level) {
        if (level.equals(FeedbackLevel.SOLUTION)) {
            ...
            if(exp != null)
                return List.of(new ASTEUni(rule.getHyp(), new ASTLiteral(exp)));
        }
