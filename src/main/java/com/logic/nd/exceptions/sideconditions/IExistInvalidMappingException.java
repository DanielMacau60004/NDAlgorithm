...
public class IExistInvalidMappingException extends NDRuleException {

    private final ASTIExist rule;
    private final ASTVariable variable;
    private final IASTExp from;
    private final IASTExp to;
    private final Set<IASTExp> outcomes;

    public IExistInvalidMappingException(ASTIExist rule, ASTVariable variable, IASTExp from, IASTExp to, Set<IASTExp> outcomes) {
    ...
    public List<IASTND> getPreviews(FeedbackLevel level) {
        if (level.equals(FeedbackLevel.SOLUTION)) {
            ...
            if(exp != null)
                return List.of(new ASTIExist(new ASTHypothesis(new ASTLiteral(exp), null), rule.getConclusion()));
        }
