...
public class IExistNotFreeVariableException extends NDRuleException {
    ...
    public IExistNotFreeVariableException(ASTIExist rule, AASTTerm term, IASTExp from, IFOLFormula to) {
    ...
    public List<IASTND> getPreviews(FeedbackLevel level) {
        if (level.equals(FeedbackLevel.SOLUTION)) {
            ...
            return List.of(new ASTIExist(new ASTHypothesis(formula, null), rule.getConclusion()));
